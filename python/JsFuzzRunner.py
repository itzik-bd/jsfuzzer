...
jsFuzzerDir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
...
runLine = "java -jar %s" % (os.path.join(jsFuzzerDir, 'jsfuzzer', 'dist', 'jsfuzzer.jar'))
paramsTemplate = "--out %%scode.js --run %s --execFlow extensive" % (os.path.join(jsFuzzerDir, 'jsfuzzer', 'engines'))
runCounter = 0
outDir = ''
...
if (__name__ == "__main__"):
	if (len(sys.argv) < 2):
		print ("usage: JsFuzzRunner.py outDir [runCounter]")
		print ("example: JsFuzzRunner.py C:\\temp")
	else:
		outDir = sys.argv[1]
	
		# Start counter at more than zero
		if (len(sys.argv) > 2):
			runCounter = int(sys.argv[2])
